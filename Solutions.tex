\documentclass[titlepage=true]{scrartcl}

\usepackage{preamble}

\begin{document}

\SSfp

\section*{Introduction} 
\addcontentsline{toc}{section}{Introduction}
    \input{Sections/Files/introduction}
\newpage

\pagenumbering{arabic}

\ihead{\footnotesize\bfseries\sffamily{Season 1, Week 1}}
\section{Season 1}

    \subsection{Week 1}
        
        \subsubsection{Intersecting Circles}
            \label{1-1-1}
            \input{Sections/Season-1/1-1-1}
        \newpage

        \subsubsection{Guava Juice}
            \label{1-1-2}
            \input{Sections/Season-1/1-1-2}
        \newpage 

        \subsubsection{Complex Roots}
            \label{1-1-3}
            \input{Sections/Season-1/1-1-3}
        \newpage 

        \subsubsection{Exponents}
            \label{1-1-4}
            \input{Sections/Season-1/1-1-4}
        \newpage

        \subsubsection{Volumes of Cubes}
            \label{1-1-5}
            \input{Sections/Season-1/1-1-5}
        \newpage

        \subsubsection{Complex Mess}
            \label{1-1-6}
            \input{Sections/Season-1/1-1-6}
        \newpage 

        \subsubsection{Paper Eating}
            \label{1-1-7}
            \input{Sections/Season-1/1-1-7}
        \newpage
        
\ihead{\footnotesize\bfseries\sffamily{Season 1, Week 2}}
\subsection{Week 2}

        \subsubsection{Regenerative Watermelons}
            \label{1-2-1}
            \input{Sections/Season-1/1-2-1}
        \newpage 

        \subsubsection{Largest Prime Factor}
            \label{1-2-2}       
            \input{Sections/Season-1/1-2-2}
        \newpage

        \subsubsection{Real Roots}
            \label{1-2-3}
            \input{Sections/Season-1/1-2-3}
        \newpage 

        \subsubsection{The Meme factor}
            \label{1-2-4}
            \input{Sections/Season-1/1-2-4}
        \newpage

        \subsubsection{Human Wolfram}
            \label{1-2-5}
            \input{Sections/Season-1/1-2-5}
        \newpage 

        \subsubsection{Guess the Config}
            \label{1-2-6}
            \input{Sections/Season-1/1-2-6}
        \newpage 
        
        \subsubsection{A Quadratic Mess}
            \label{1-2-7}
            \input{Sections/Season-1/1-2-7}
        \newpage

\ihead{\footnotesize\bfseries\sffamily{.19's Season (Season 2), Week 1}}
\section{Season 2 (.19's Season)}

    \subsection{Week 1}

        \subsubsection{A Sequence of 5's}
            \label{2-1-1}
            \input{Sections/Season-2/2-1-1}
        \newpage

        \subsubsection{Brainy's Happy Set}
            \label{2-1-2}
            \input{Sections/Season-2/2-1-2}
        \newpage
       
        \subsubsection{MODSbot's Escape!}
            \label{2-1-3}
            \input{Sections/Season-2/2-1-3}        
        \newpage 

        \subsubsection{Sides of a Polygon}
            \label{2-1-4}
            \input{Sections/Season-2/2-1-4}
        \newpage 

        \subsubsection{2p}
            \label{2-1-5}
            \input{Sections/Season-2/2-1-5}
        \newpage

        \subsubsection{Slippery Rooks}
            \label{2-1-6}
            \input{Sections/Season-2/2-1-6}
        \newpage
            
        \subsubsection{Sets of Integer Solutions}
            \label{2-1-7}
            \input{Sections/Season-2/2-1-7}        
        \newpage 

\ihead{\footnotesize\bfseries\sffamily{.19's Season (Season 2), Week 2}}
    \subsection{Week 2}
    
        \subsubsection{A Game of Deductions}
            \label{2-2-1}
            \input{Sections/Season-2/2-2-1}
        \newpage

        \subsubsection{Maximising Exponents}
            \label{2-2-2}
            \input{Sections/Season-2/2-2-2}
        \newpage
            
        \subsubsection{Colourful Problem}
            \label{2-2-3}
            \input{Sections/Season-2/2-2-3}    
        \newpage 
        
        \subsubsection{Combinatoral Addition}
            \label{2-2-4}
            \input{Sections/Season-2/2-2-4}
        \newpage 

        \subsubsection{Expected Value}
            \label{2-2-5}
            \input{Sections/Season-2/2-2-5}
        \newpage

        \subsubsection{Infinite Product}
            \label{2-2-6}
            \input{Sections/Season-2/2-2-6}
        \newpage

        \subsubsection{Projective Geo}
            \label{2-2-7}
            \input{Sections/Season-2/2-2-7}
        \newpage 

\ihead{\footnotesize\bfseries\sffamily{Trigonometric Troubles (Season 3), Week 1}}
\section{Trigonometric Troubles (Season 3)}

    \subsection{Week 1}
        
        \subsubsection{Maximising Trig. Function}
            \label{3-1-1}
            \input{Sections/Season-3/3-1-1}
        \newpage
        
        \subsubsection{Areas inside a square}
            \label{3-1-2}
            \input{Sections/Season-3/3-1-2}
        \newpage
        
        \subsubsection{Length of SQ}
            \label{3-1-3}
            \input{Sections/Season-3/3-1-3}
        \newpage
                
        \subsubsection{Sum of Tan's}
            \label{3-1-4}
            \input{Sections/Season-3/3-1-4}
        \newpage
        
        \subsubsection{Distance from the Orthocentre}
            \label{3-1-5}
            \input{Sections/Season-3/3-1-5}
        \newpage 
        
        \subsubsection{Minimising the Diagonal}
             \label{3-1-6}
            \input{Sections/Season-3/3-1-6}
        \newpage
        
        \subsubsection{Circumcentre}
            \label{3-1-7}
            \input{Sections/Season-3/3-1-7}
        \newpage
        
\ihead{\footnotesize\bfseries\sffamily{Piboi's Bashy Combo (Season 4) \textbf{VOIDED}}}
\section{Piboi's Bashy Combo (Season 4) \textbf[VOIDED]}
        
    \input{Sections/Files/Season-4-Statement}
    \newpage 
        
    ``\emph{This is a document of combinatorical problems of which some are original, and some are modified. I'm very sorry if there are any mistakes. Please let me know if there are any on my AoPS \textbf{(Ultraman)} or my discord \textbf{(Charge\#3766)}. Enjoy!}"\\
        
    \begin{center}
        \textbf{Note: The following solutions and answers, where applicable, may not be correct}
    \end{center}
  
    \ihead{\footnotesize\bfseries\sffamily{Piboi's Bashy Combo (Season 4), Week 1}}
    \subsection{Week 1}
                
        \subsubsection{A Tricky Shuffle \textbf[VOIDED]}
            \label{4-1-1}
            \input{Sections/Season-4/4-1-1}
        \newpage 
                
        \subsubsection{Coloured Markers \textbf[VOIDED]}
            \label{4-1-2}
            \input{Sections/Season-4/4-1-2}
        \newpage 
        
        \subsubsection{Common Names \textbf[VOIDED]}
            \label{4-1-3}
            \input{Sections/Season-4/4-1-3}
        \newpage 
        
        \subsubsection{Funny Questions \textbf[VOIDED]}
            \label{4-1-4}
            \input{Sections/Season-4/4-1-4}
        \newpage 
    
        \subsubsection{Colourful Integers \textbf[VOIDED]}
            \label{4-1-5}
            \input{Sections/Season-4/4-1-5}
        \newpage 

\ihead{\footnotesize\bfseries\sffamily{Back to School (Season 5)}}
\section{Back to School (Season 5) }
            
    \ihead{\footnotesize\bfseries\sffamily{Back to School (Season 5), Week 1}}
    \subsection{Week 1}
    
        \subsubsection{A Tricky Combination}
            \label{5-1-1}
            \input{Sections/Season-5/5-1-1}
        \newpage 

        \subsubsection{Geometric Sequence}
            \label{5-1-2}
            \input{Sections/Season-5/5-1-2}
        \newpage 

        \subsubsection{Simultaneous Equation?}
            \label{5-1-3}
            \input{Sections/Season-5/5-1-3}
        \newpage

        \subsubsection{Tangent Circles}
            \label{5-1-4}
            \input{Sections/Season-5/5-1-4}
        \newpage

        \subsubsection{Santa's Elves}
            \label{5-1-5}
            \input{Sections/Season-5/5-1-5}
        \newpage

        \subsubsection{2016 Algebra}
            \label{5-1-6}
            \input{Sections/Season-5/5-1-6}
        \newpage

        \subsubsection{\(x^y\)'s}
            \label{5-1-7}
            \input{Sections/Season-5/5-1-7}
        \newpage
    
    \ihead{\footnotesize\bfseries\sffamily{Back to School (Season 5), Week 2}}
    \subsection{Week 2}

        \subsubsection{Pentagons}
            \label{5-2-1}
            \input{Sections/Season-5/5-2-1}
        \newpage

        \subsubsection{Digits in a String}
            \label{5-2-2}
            \input{Sections/Season-5/5-2-2}
        \newpage

        \subsubsection{Relatively Prime Function}
            \label{5-2-3}
            \input{Sections/Season-5/5-2-3}
        \newpage

        \subsubsection{Intersecting Circles}
            \label{5-2-4}
            \input{Sections/Season-5/5-2-4}
        \newpage

        \subsubsection{Discord Ping Fight}
            \label{5-2-5}
            \input{Sections/Season-5/5-2-5}
        \newpage

        \subsubsection{Falling Cards}
            \label{5-2-6}
            \input{Sections/Season-5/5-2-6}
        \newpage

        \subsubsection{Computing the Area of a Triangle}
            \label{5-2-7}
            \input{Sections/Season-5/5-2-7}
        \newpage

\ihead{\footnotesize\bfseries\sffamily{Third Week of School (Season 6)}}
\section{Third Week of School (Season 6) }
                    
    \ihead{\footnotesize\bfseries\sffamily{Third Week of School (Season 6), Week 1}}
    \subsection{Week 1}

        \subsubsection{Maximising Square Factors}
            \label{6-1-1}
            \input{Sections/Season-6/6-1-1}
        \newpage

        \subsubsection{Njoy's Balls}
            \label{6-1-2}
            \input{Sections/Season-6/6-1-2}
        \newpage

        \subsubsection{Aiya's Function}
            \label{6-1-3}
            \input{Sections/Season-6/6-1-3}
        \newpage

        \subsubsection{The AIME Cyclic}
            \label{6-1-4}
            \input{Sections/Season-6/6-1-4}
        \newpage

        \subsubsection{Prime Floors}
            \label{6-1-5}
            \input{Sections/Season-6/6-1-5}
        \newpage

        \subsubsection{Nested Periodic Functions}
            \label{6-1-6}
            \input{Sections/Season-6/6-1-6}

        \subsubsection{Braniy's Party}
            \label{6-1-7}
            \input{Sections/Season-6/6-1-7}
        \newpage

    \ihead{\footnotesize\bfseries\sffamily{Third Week of School (Season 6), Week 2}}
    \subsection{Week 2}

        \subsubsection{Negative Sum}
            \label{6-2-1}
            \input{Sections/Season-6/6-2-1}
        \newpage

        \subsubsection{Doubling Digit Sum}
            \label{6-2-2}
            \input{Sections/Season-6/6-2-2}
        \newpage

        \subsubsection{Tony Vs. Wang}
            \label{6-2-3}
            \input{Sections/Season-6/6-2-3}
        \newpage 
        
        \subsubsection{Equilateral Decagon}
            \label{6-2-4}
            \input{Sections/Season-6/6-2-4}
        \newpage

        % P5 A
        \subsubsection{Triangles on a Cubic} 
            \label{6-2-5}
            \input{Sections/Season-6/6-2-5}
        \newpage

        % P6 C    
        \subsubsection{Jumping Frogs}
            \label{6-2-6}
            \input{Sections/Season-6/6-2-6}
        \newpage 

        \subsubsection{Minimising Ratios}
            \label{6-2-7}
            \input{Sections/Season-6/6-2-7}
        \newpage

\ihead{\footnotesize\bfseries\sffamily{PSC's Adventure (Season 7)}}
\section{PSC's Adventure (Season 7)}

	\ihead{\footnotesize\bfseries\sffamily{PSC's Adventure (Season 7), Week 1}}
	\subsection{Week 1}
	
        \subsubsection{The Jungle Polygon}
            \label{7-1-1}
            \input{Sections/Season-7/7-1-1}
            \newpage
            
            \subsubsection{Uphill and downhill}
            \label{7-1-2}  
            \input{Sections/Season-7/7-1-2}
        \newpage

        \subsubsection{Just a Beauty!}
            \label{7-1-3}  
            \input{Sections/Season-7/7-1-3}
        \newpage

        \subsubsection{Probability of Intervals}
            \label{7-1-4}  
            \input{Sections/Season-7/7-1-4}
        \newpage

        \subsubsection{Dividing Functions}
            \label{7.1.5}  
            \input{Sections/Season-7/7-1-5}
        \newpage

        \subsubsection{Paper Monster}
            \label{7.1.6}  
            \input{Sections/Season-7/7-1-6}
        \newpage
        
        \subsubsection{Tan's Optimisation}
            \label{7.1.7}  
            \input{Sections/Season-7/7-1-7}
        \newpage
        
	\ihead{\footnotesize\bfseries\sffamily{PSC's Adventure (Season 7), Week 2}}
    \subsection{Week 2}
        
        \subsubsection{A Digital Product equal to Half the Number}
        	\label{7.2.1}  
        	\input{Sections/Season-7/7-2-1}
        \newpage
        
        \subsubsection{Unique Odd Numbers}
        	\label{7.2.2}  
        	\input{Sections/Season-7/7-2-2}
        \newpage

        \subsubsection{Maximising \(a^4+b^4\)}
            \label{7.2.3}  
            \input{Sections/Season-7/7-2-3}
        \newpage    

        \subsubsection{Minimising Perimeter}
            \label{7.2.4}
            \input{Sections/Season-7/7-2-4}
        \newpage 

        \subsubsection{Binary Blocks}
            \label{7.2.5}  
            \input{Sections/Season-7/7-2-5}
        \newpage

        \subsubsection{Funkey Triangles}
            \label{7.2.6}  
            \input{Sections/Season-7/7-2-6}
        \newpage

        \subsubsection{A Lotta Touples!}
            \label{7.2.7}  
            \input{Sections/Season-7/7-2-7}
        \newpage
        
\ihead{\footnotesize\bfseries\sffamily{A New Chapter (Season 8)}}
\section{A New Chapter (Season 8)}
        
    \ihead{\footnotesize\bfseries\sffamily{A New Chapter (Season 8), Week 1}}
    \subsection{Week 1}

        \subsubsection{Smallest Non-Factor}
            \label{8.1.1}
            \input{Sections/Season-8/8-1-1}
        \newpage
        
        \subsubsection{Product of Radii}
            \label{8.1.2}  
            \input{Sections/Season-8/8-1-2}
        \newpage

        \subsubsection{Two-to-One Functions}
            \label{8.1.3}  
            \input{Sections/Season-8/8-1-3}
        \newpage

        \subsubsection{Modular Powers}
            \label{8.1.4}  
            \input{Sections/Season-8/8-1-4}
        \newpage

        \subsubsection{Australian Nim}
            \label{8.1.5}  
            \input{Sections/Season-8/8-1-5}
        \newpage

        \subsubsection{Another Geo Config}
            \label{8.1.6}  
            \input{Sections/Season-8/8-1-6}
        \newpage

        \subsubsection{Product of Root Differences}
            \label{8.1.7}  
            \input{Sections/Season-8/8-1-7}
        \newpage


    \ihead{\footnotesize\bfseries\sffamily{A New Chapter (Season 8), Week 2}}
    \subsection{Week 2}
    
        \subsubsection{Counting Intersections}
            \label{8.2.1}  
            \input{Sections/Season-8/8-2-1}
        \newpage
    
        \subsubsection{The Race}
            \label{8.2.2}  
            \input{Sections/Season-8/8-2-2}
        \newpage

        \subsubsection{Remainder of Sums of Squares}
            \label{8.2.3}  
            \input{Sections/Season-8/8-2-3}
        \newpage

        \subsubsection{Another 255 Subsets}
            \label{8.2.4}
            \input{Sections/Season-8/8-2-4}
        \newpage 

        \subsubsection{4-5-6 Triangle}
            \label{8.2.5}  
            \input{Sections/Season-8/8-2-5}
        \newpage

        \subsubsection{``What's a signed integer?"}
            \label{8.2.6}
            \input{Sections/Season-8/8-2-6}
        \newpage

        \subsubsection{Diagonals in a Convex 1001-gon}
            \label{8.2.7}  
            \input{Sections/Season-8/8-2-7}
        \newpage

\setcounter{section}{9}
\ihead{\footnotesize\bfseries\sffamily{CCCC After Math (Season 10)}}
\section{CCCC After Math (Season 10)}

    \ihead{\footnotesize\bfseries\sffamily{CCCC After Math (Season 10), Week 1}}
    \subsection{Week 1}

        \subsubsection{Epic\textcolor{red}{X}troll}
        \label{10.1.1}
        \input{Sections/Season-10/10-1-1}
        \newpage

        \subsubsection{Similar Triangles}
        \label{10.1.2}  
        \input{Sections/Season-10/10-1-2}
        \newpage

        \subsubsection{Factorial Sums and Divisors}
        \label{10.1.3}  
        \input{Sections/Season-10/10-1-3}
        \newpage

        \subsubsection{Brainy's Passcode}
        \label{10.1.4}  
        \input{Sections/Season-10/10-1-4}
        \newpage

        \subsubsection{Production Loop}
        \label{10.1.5}  
        \input{Sections/Season-10/10-1-5}
        \newpage

        \subsubsection{Weird polynomial}
        \label{10.1.6}  
        \input{Sections/Season-10/10-1-6}
        \newpage
		
		\subsubsection{Weird triangle}
		\label{10.1.7}
		\input{Sections/Season-10/10-1-7}
		\newpage

    \ihead{\footnotesize\bfseries\sffamily{CCCC After Math (Season 10), Week 2}}
    \subsection{Week 2}

        \subsubsection{Real Square Roots}
        \label{10.2.1}  
        \input{Sections/Season-10/10-2-1}
        \newpage

        \subsubsection{"What's a positive integer?"}
        \label{10.2.2}  
        \input{Sections/Season-10/10-2-2}
        \newpage
			
        \subsubsection{Converging Tangent Circles}
        \label{10.2.3}  
        \input{Sections/Season-10/10-2-3}
        \newpage

        \subsubsection{Maze Escape}
        \label{10.2.4}
        \input{Sections/Season-10/10-2-4}
        \newpage 

        \subsubsection{Primes divide powers of 5}
        \label{10.2.5}  
        \input{Sections/Season-10/10-2-5}
        \newpage

        \subsubsection{tRiViaL}
        \label{10.2.6}  
        \input{Sections/Season-10/10-2-6}
        \newpage

        \subsubsection{Lamps in a Circle}
        \label{10.2.7}  
        \input{Sections/Season-10/10-2-7}
        \newpage

\ihead{\footnotesize\bfseries\sffamily{err¶¶: ¶¶ ¶¶¶ ¶¶¶¶¶¶¶ ¶¶¶¶ ¶¶ ¶¶¶ ¶¶¶¶ ¶¶¶¶¶. (Season 11)}}
\section{err¶¶: ¶¶ ¶¶¶ ¶¶¶¶¶¶¶ ¶¶¶¶ ¶¶ ¶¶¶ ¶¶¶¶ ¶¶¶¶¶. (Season 11)}

    \ihead{\footnotesize\bfseries\sffamily{err¶¶: ¶¶ ¶¶¶ ¶¶¶¶¶¶¶ ¶¶¶¶ ¶¶ ¶¶¶ ¶¶¶¶ ¶¶¶¶¶. (Season 11), Week 1}}
    \subsection{Week 1}

        \subsubsection{Integral AM-GM}
        \label{11.1.1}
        \input{Sections/Season-11/11-1-1.tex}
        \newpage

        \subsubsection{d20 but not D20}
        \label{11.1.2}
        \input{Sections/Season-11/11-1-2.tex}
        \newpage

        \subsubsection{Is it better to walk or run in the rain?}
        \label{11.1.3}
        \input{Sections/Season-11/11-1-3.tex}
        \newpage

        \subsubsection{Don't over think this}
        \label{11.1.4}
        \input{Sections/Season-11/11-1-4.tex}
        \newpage
        
        \subsubsection{Genfun is Not Fun}
        \label{11.1.5}
        \input{Sections/Season-11/11-1-5.tex}
        \newpage

        \subsubsection{Harmonic Sum}
        \label{11.1.6}
        \input{Sections/Season-11/11-1-6.tex}
        \newpage

        \subsubsection{Lonely Tetrahedron}
        \label{11.1.7}
        \input{Sections/Season-11/11-1-7.tex}
        \newpage

    \ihead{\footnotesize\bfseries\sffamily{err¶¶: ¶¶ ¶¶¶ ¶¶¶¶¶¶¶ ¶¶¶¶ ¶¶ ¶¶¶ ¶¶¶¶ ¶¶¶¶¶. (Season 11), Week 2}}
    \subsection{Week 2}

        \subsubsection{Don't over think this part 2}
        \label{11.2.1}
        \input{Sections/Season-11/11-2-1.tex}
        \newpage

        \subsubsection{Lattice Points on a Circle}
        \label{11.2.2}
        \input{Sections/Season-11/11-2-2.tex}
        \newpage

        \subsubsection{WA is Required}
        \label{11.2.3}
        \input{Sections/Season-11/11-2-3.tex}
        \newpage

        \subsubsection{Hard}
        \label{11.2.4}
        \input{Sections/Season-11/11-2-4.tex}
        \newpage

        % \subsubsection{hard}
        % \label{11.2.5}
        % \input{Sections/Season-11/11-2-5.tex}
        % \newpage

\ihead{\footnotesize\bfseries\sffamily{Return to Normalcy (Season 12)}}
\section{Return to Normalcy (Season 12)}

    \ihead{\footnotesize\bfseries\sffamily{Return to Normalcy (Season 12), Week 1}}
    \subsection{Week 1}
        
    \subsubsection{Quadrilateral Perimeter}
	\label{12.1.1}
	\input{Sections/Season-12/orthogonal-quadrilateral.tex}
	\newpage

    \subsubsection{Involution Convolution}
	\label{12.1.2}
	\input{Sections/Season-12/AusMCS-2019-30.tex}
	\newpage

    \subsubsection{Coordinates without Bash}
	\label{12.1.3}
	\input{Sections/Season-12/Denial-1.tex}
	\newpage

    \subsubsection{April Fool's}
	\label{12.1.4}
	\input{Sections/Season-12/April-fools.tex}
	\newpage
	
	\subsubsection{FE}
	\label{12.1.5}
	\input{Sections/Season-12/Kosovo-TST-2011-5.tex}
	\newpage

	\subsubsection{So this is where American problem quality went}
	\label{12.1.6}
	\input{Sections/Season-12/AIME-2012-15.tex}
	\newpage

	\subsubsection{640 Generators of Pain}
	\label{12.1.7}
	\input{Sections/Season-12/PUMAC-2015-NA7.tex}
	\newpage

    \ihead{\footnotesize\bfseries\sffamily{Return to Normalcy (Season 12), Week 2}}
    \subsection{Week 2}

    \subsubsection{Even Numbers with Restrictions}
	\label{12.2.1}
	\input{Sections/Season-12/AMC10-2011-Q13.tex}
	\newpage
    
    \subsubsection{Enjoyable}
	\label{12.2.2}
	\input{Sections/Season-12/COMC-2018.tex}
	\newpage

    \subsubsection{Isosceles Tetrahedron}
	\label{12.2.3}
	\input{Sections/Season-12/IMTS-1991-R4P5.tex}
	\newpage

    \subsubsection{Sums of powers of 2 but weird}
	\label{12.2.4}
	\input{Sections/Season-12/PUMAC-2018-AA5.tex}
	\newpage

	\subsubsection{"D4 I swear"}
	\label{12.2.5}
	\input{Sections/Season-12/OMO-2012-Winter-43.tex}
	\newpage

    \subsubsection{"change the point distribution up so weekends don't carry people"}
	\label{12.2.6}
	\input{Sections/Season-12/Yay-CG.tex}
	\newpage

	\subsubsection{Notice me, sqing-senpai}
	\label{12.2.7}
	\input{Sections/Season-12/Notice-me-sqing-senpai.tex}
	\newpage

\ihead{\footnotesize\bfseries\sffamily{Quality Uncontrol (Season 13)}}
\section{Quality Uncontrol (Season 13)}

    \ihead{\footnotesize\bfseries\sffamily{Quality Uncontrol (Season 13), Week 1}}
    \subsection{Week 1}
        
    \subsubsection{Choi's Balls}
	\label{13.1.1}
	\input{Sections/Season-13/13-1-1}
	\newpage

    \subsubsection{Biscriminant}
	\label{13.1.2}
	\input{Sections/Season-13/13-1-2}
	\newpage
    
    \subsubsection{\Cmac  2012 P6}
	\label{13.1.3}
	\input{Sections/Season-13/13-1-3}
	\newpage

    \subsubsection{NICE Geometry}
	\label{13.1.4}
	\input{Sections/Season-13/13-1-4}
	\newpage

    \subsubsection{Graph Theory in Disguise}
	\label{13.1.5}
	\input{Sections/Season-13/13-1-5}
	\newpage

    \subsubsection{What the H-E-double hockey sticks?}
	\label{13.1.6}
	\input{Sections/Season-13/13-1-6}
	\newpage

    \subsubsection{"What's an mgf?"}
	\label{13.1.7}
	\input{Sections/Season-13/13-1-7}
	\newpage

\ihead{\footnotesize\bfseries\sffamily{Quality Uncontrol (Season 13), Week 2}}
\subsection{Week 2}

    \subsubsection{It's Okay :)}
    \label{13.2.1}
    \input{Sections/Season-13/13-2-1}
    \newpage

    \subsubsection{Points on a Line}
	\label{13.2.2}
	\input{Sections/Season-13/13-2-2}
	\newpage

    \subsubsection{Diamond Operator}
	\label{13.2.3}
	\input{Sections/Season-13/13-2-3}
	\newpage
    
    \subsubsection{Weird Wilson}
	\label{13.2.4}
	\input{Sections/Season-13/13-2-4}
	\newpage

    \subsubsection{Get the CRUX of this inequality!}
	\label{13.2.5}
	\input{Sections/Season-13/13-2-5}
	\newpage

    \subsubsection{Don't overthink this}
	\label{13.2.6}
	\input{Sections/Season-13/13-2-6}
	\newpage

    \subsubsection{The Party, The Politburo, and the PSC}
	\label{13.2.7}
	\input{Sections/Season-13/13-2-7}
	\newpage

\ihead{\footnotesize\bfseries\sffamily{Mityushikha Bay (Season 14)}}
\section{Mityushikha Bay (Season 14)}

    \ihead{\footnotesize\bfseries\sffamily{Mityushikha Bay (Season 14), Week 1}}
    \subsection{Week 1}

    \subsubsection{Rectangle Rotation}
	\label{14.1.1}
	\input{Sections/Season-14/1-1}
	\newpage

	\subsubsection{NJOY Troll (again)}
	\label{14.1.2}
	\input{Sections/Season-14/1-2}
	\newpage

	\subsubsection{You Just Got Vector'ed}
	\label{14.1.3}
	\input{Sections/Season-14/1-3}
	\newpage

	\subsubsection{Answer Extraction}
	\label{14.1.4}
	\input{Sections/Season-14/1-4}
	\newpage

    \subsubsection{Not a Fan}
	\label{14.1.5}
	\input{Sections/Season-14/1-5}
	\newpage

	\subsubsection{you're welcome, brainy}
	\label{14.1.6}
	\input{Sections/Season-14/1-6}
	\newpage

	\subsubsection{RDS-1}
	\label{14.1.7}
	\input{Sections/Season-14/1-7}
	\newpage
	
	\ihead{\footnotesize\bfseries\sffamily{Mityushikha Bay (Season 14), Week 2}}
    \subsection{Week 2}
	
	\subsubsection{All About that Base}
	\label{14.2.1}
	\input{Sections/Season-14/2-1}
	\newpage

	\subsubsection{Number of QoTD Seasons}
	\label{14.2.2}
	\input{Sections/Season-14/2-2}
	\newpage

	\subsubsection{Square and Point}
	\label{14.2.3}
	\input{Sections/Season-14/2-3}
	\newpage

	\subsubsection{2012 HMMT A6}
	\label{14.2.4}
	\input{Sections/Season-14/2-4}
	\newpage

	\subsubsection{[under construction]}
	\label{14.2.5}
	% \input{Sections/Season-14/2-5}
	\newpage

	\subsubsection{Projective Pain}
	\label{14.2.6}
	\input{Sections/Season-14/2-6}
	\newpage

	\subsubsection{AN602}
	\label{14.2.7}
	\input{Sections/Season-14/2-7}
	\newpage

\ihead{\footnotesize\bfseries\sffamily{SALT-1 (Season 15)}}
\section{SALT-1 (Season 15)}

    \ihead{\footnotesize\bfseries\sffamily{SALT-1 (Season 15), Week 1}}
    \subsection{Week 1}

    \subsubsection{"What are distinct integers?"}
	\label{15.1.1}
	\input{Sections/Season-15/1-1}
	\newpage

	\subsubsection{Troll Pigeonhole}
	\label{15.1.2}
	\input{Sections/Season-15/1-2}
	\newpage

    \subsubsection{WA Can't Solve This}
	\label{15.1.3}
	\input{Sections/Season-15/1-3}
	\newpage

    \subsubsection{Prime Product}
	\label{15.1.4}
	\input{Sections/Season-15/1-4}
	\newpage
	
	\subsubsection{The Rare Non-Horrible States Problem}
	\label{15.1.5}
	\input{Sections/Season-15/1-5}
	\newpage

	\subsubsection{Minimal Area, Minimal Difficulty for a Saturday}
	\label{15.1.6}
	\input{Sections/Season-15/1-6}
	\newpage

	\subsubsection{Detente}
	\label{15.1.7}
	\input{Sections/Season-15/1-7}
	\newpage
    
    \ihead{\footnotesize\bfseries\sffamily{SALT-1 (Season 15), Week 2}}
    \subsection{Week 2}
    
    \subsubsection{Senior Math; What Challenge?}
	\label{15.2.1}
	\input{Sections/Season-15/2-1}
	\newpage

	\subsubsection{Prisoner's Stupid Dilemma}
	\label{15.2.2}
	\input{Sections/Season-15/2-2}
	\newpage

	\subsubsection{Coordinates without Bash}
	\label{15.2.3}
	\input{Sections/Season-15/2-3}
	\newpage

	\subsubsection{WA Can't Solve This, Either}
	\label{15.2.4}
	\input{Sections/Season-15/2-4}
	\newpage

	\subsubsection{WA Can't Solve This, Either}
	\label{15.2.5}
	\input{Sections/Season-15/2-5}
	\newpage

	\subsubsection{cHriS geO}
	\label{15.2.6}
	\input{Sections/Season-15/2-6}
	\newpage

	\subsubsection{Treaty Violation}
	\label{15.2.7}
	\input{Sections/Season-15/2-7}
	\newpage

\ihead{\footnotesize\bfseries\sffamily{Quality Control (Season 16)}}
\section{Quality Control (Season 16)}

    \ihead{\footnotesize\bfseries\sffamily{Quality Control (Season 16), Week 1}}
    \subsection{Week 1}

    \subsubsection{The Rare non-Troll NJOY Problem}
	\label{16.1.1}
	\input{Sections/Season-16/1-1}
	\newpage

	\subsubsection{The Common Troll NJOY Problem}
	\label{16.1.2}
	\input{Sections/Season-16/1-2}
	\newpage

	\subsubsection{Parallelogram PTSD}
	\label{16.1.3}
	\input{Sections/Season-16/1-3}
	\newpage

	\subsubsection{WA Could Solve This and you Still Failed}
	\label{16.1.4}
	\input{Sections/Season-16/1-4}
	\newpage

    \subsubsection{Surprising System}
	\label{16.1.5}
	\input{Sections/Season-16/1-5}
	\newpage

	\subsubsection{Really Weird NT}
	\label{16.1.6}
	\input{Sections/Season-16/1-6}
	\newpage

    \subsubsection{You just Lost The Game}
	\label{16.1.7}
	\input{Sections/Season-16/1-7}
	\newpage
    
    \ihead{\footnotesize\bfseries\sffamily{Quality Control (Season 16), Week 2}}
    \subsection{Week 2}
    
    \subsubsection{Honestly, Too Much of a Beginner Problem}
	\label{16.2.1}
	\input{Sections/Season-16/2-1}
	\newpage

	\subsubsection{Someone Leaked the Answer and you Still Failed}
	\label{16.2.2}
	\input{Sections/Season-16/2-2}
	\newpage

    \subsubsection{Non-Chris Geo}
	\label{16.2.3}
	\input{Sections/Season-16/2-3}
	\newpage

    \subsubsection{WA could Solve This and you Failed Again!}
	\label{16.2.4}
	\input{Sections/Season-16/2-4}
	\newpage

    \subsubsection{d20(20) but not D20}
	\label{16.2.5}
	\input{Sections/Season-16/2-5}
	\newpage

    \subsubsection{That's Not LTE}
	\label{16.2.6}
	\input{Sections/Season-16/2-6}
	\newpage

	\subsubsection{Genfun is Not Fun, v2}
	\label{16.2.7}
	\input{Sections/Season-16/2-7}
	\newpage

{\ihead{\footnotesize\bfseries\sffamily{Testsolver's Adventure (Season 17)}}
\section{Testsolver's Adventure (Season 17)}

    \ihead{\footnotesize\bfseries\sffamily{Testsolver's Adventure (Season 17), Week 1}}
    \subsection{Week 1}

    \subsubsection{Indian Remainder Theorem}
    \label{17.1.1}
    \input{Sections/Season-17/1-1}
    \newpage

    \subsubsection{The Rare non-NJOY Tuesday}
	\label{17.1.2}
	\input{Sections/Season-17/1-2}
	\newpage

    \subsubsection{Periodic Function}
	\label{17.1.3}
	\input{Sections/Season-17/1-3}
	\newpage

    \subsubsection{I don't feel like putting the flavourtexted version up here}
	\label{17.1.4}
	\input{Sections/Season-17/1-4}
	\newpage
	
	\subsubsection{Wang is at the Center of Everything that Happens to Me}
	\label{17.1.5}
	\input{Sections/Season-17/1-5}
	\newpage

    \setcounter{subsubsection}{6}
	\subsubsection{MODS Logo}
	\label{17.1.7}
	\input{Sections/Season-17/1-7}
	\newpage
    
    \ihead{\footnotesize\bfseries\sffamily{Testsolver's Adventure (Season 17), Week 2}}
    \subsection{Week 2}

    \subsubsection{The Difficulty of an IMO Training Camp}
    \label{17.2.1}
    \input{Sections/Season-17/2-1}
    \newpage

    \subsubsection{The Rare non-NJOY Tuesday}
	\label{17.2.2}
	\input{Sections/Season-17/2-2}
	\newpage

    \subsubsection{Partial Fractions is only a Partial Solution}
	\label{17.2.3}
	\input{Sections/Season-17/2-3}
	\newpage

    \subsubsection{Who had the Courage to Coordbash This?}
	\label{17.2.4}
	\input{Sections/Season-17/2-4}
	\newpage
	
	\subsubsection{"not hard enough for a weekend"}
	\label{17.2.5}
	\input{Sections/Season-17/2-5}
	\newpage

	\subsubsection{Fat Tan}
	\label{17.2.6}
	\input{Sections/Season-17/2-6}
	\newpage

    \subsubsection{I had brain(y) drain after solving W2P6}
	\label{17.2.7}
	\input{Sections/Season-17/2-7}
	\newpage

{\ihead{\footnotesize\bfseries\sffamily{OpenPOTD Games (Season 18)}}
\section{OpenPOTD Games (Season 18)}

    \ihead{\footnotesize\bfseries\sffamily{OpenPOTD Games (Season 18), Week 1}}
    \subsection{Week 1}

    \subsubsection{Don't overthink this pt.3}
    \label{18.1.1}
    \input{Sections/Season-18/1-1}
    \newpage

    \subsubsection{You overthought this, didn't you?}
    \label{18.1.2}
    \input{Sections/Season-18/1-2}
    \newpage

    \subsubsection{Oh no, I drew the short...ball?}
    \label{18.1.3}
    \input{Sections/Season-18/1-3}
    \newpage

    \subsubsection{Finally, a Non-Guessable FE}
    \label{18.1.4}
    \input{Sections/Season-18/1-4}
    \newpage
    
    \subsubsection{Chris, but Hardly Geo}
    \label{18.1.5}
    \input{Sections/Season-18/1-5}
    \newpage

    \subsubsection{answer extraction why}
    \label{18.1.6}
    \input{Sections/Season-18/1-6}
    \newpage

    % \subsubsection{MODS Logo}
    % \label{18.1.7}
    % \input{Sections/Season-18/1-7}
    % \newpage
    
    % \ihead{\footnotesize\bfseries\sffamily{OpenPOTD Games (Season 18), Week 2}}
    % \subsection{Week 2}

    % \subsubsection{The Difficulty of an IMO Training Camp}
    % \label{18.2.1}
    % \input{Sections/Season-18/2-1}
    % \newpage

    % \subsubsection{The Rare non-NJOY Tuesday}
    % \label{18.2.2}
    % \input{Sections/Season-18/2-2}
    % \newpage

    % \subsubsection{Partial Fractions is only a Partial Solution}
    % \label{18.2.3}
    % \input{Sections/Season-18/2-3}
    % \newpage

    % \subsubsection{Who had the Courage to Coordbash This?}
    % \label{18.2.4}
    % \input{Sections/Season-18/2-4}
    % \newpage
    
    % \subsubsection{"not hard enough for a weekend"}
    % \label{18.2.5}
    % \input{Sections/Season-18/2-5}
    % \newpage

    % \subsubsection{Fat Tan}
    % \label{18.2.6}
    % \input{Sections/Season-18/2-6}
    % \newpage

    % \subsubsection{I had brain(y) drain after solving W2P6}
    % \label{18.2.7}
    % \input{Sections/Season-18/2-7}
    % \newpage

\setcounter{section}{21}
    {\ihead{\footnotesize\bfseries\sffamily{Season '22 (Season 22)}}
\section{Season '22 (Season 22)}

    \ihead{\footnotesize\bfseries\sffamily{Season '22 (Season 22), Week 1}}
    \subsection{Week 1}

    \subsubsection{Don't overthink this pt.4}
    \label{22.1.1}
    \input{Sections/Season-22/1-1}
    \newpage

    \subsubsection{No Trig Identities necessary}
    \label{22.1.2}
    \input{Sections/Season-22/1-2}
    \newpage

    \subsubsection{Pascal's Fractions}
    \label{22.1.3}
    \input{Sections/Season-22/1-3}
    \newpage

    \subsubsection{Closet}
    \label{22.1.4}
    \input{Sections/Season-22/1-4}
    \newpage
    
    \subsubsection{Object Class: Safe}
    \label{22.1.5}
    \input{Sections/Season-22/1-5}
    \newpage

    \subsubsection{10\$ to anyone who can gf this}
    \label{22.1.6}
    \input{Sections/Season-22/1-6}
    \newpage

    \subsubsection{Spectral Snakes}
    \label{22.1.7}
    \input{Sections/Season-22/1-7}
    \newpage
    
    \ihead{\footnotesize\bfseries\sffamily{Season '22 (Season 22), Week 2}}
    \subsection{Week 2}

    \subsubsection{i feel like we've done the exact same trick before}
    \label{22.2.1}
    \input{Sections/Season-22/2-1}
    \newpage

    % \subsubsection{The Rare non-NJOY Tuesday}
    % \label{22.2.2}
    % \input{Sections/Season-22/2-2}
    % \newpage

    % \subsubsection{Partial Fractions is only a Partial Solution}
    % \label{22.2.3}
    % \input{Sections/Season-22/2-3}
    % \newpage

    % \subsubsection{Who had the Courage to Coordbash This?}
    % \label{22.2.4}
    % \input{Sections/Season-22/2-4}
    % \newpage
    
    % \subsubsection{"not hard enough for a weekend"}
    % \label{22.2.5}
    % \input{Sections/Season-22/2-5}
    % \newpage

    % \subsubsection{Fat Tan}
    % \label{22.2.6}
    % \input{Sections/Season-22/2-6}
    % \newpage

    % \subsubsection{I had brain(y) drain after solving W2P6}
    % \label{22.2.7}
    % \input{Sections/Season-22/2-7}
    % \newpage

\end{document}